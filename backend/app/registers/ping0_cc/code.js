...
// 框架运行内存
catvm.memory = {
    config: {print: false, proxy: true}, // 框架配置：是否打印，是否使用proxy
...
window = {}; // 伪造window对象，window是全局的，因此我们需要伪造一个全局对象
// debugger;
...
window.plugins = catvm.memory.PluginArray._;
window.PluginArray = catvm.memory.PluginArray._;
// 存储一些值，避免污染全局变量空间
...
document.documentElement = function documentElement() {
    var tagElement = catvm.memory.htmlelements['html']();
    return catvm.proxy(tagElement);
}
...
catvm.safefunction(document.hasFocus)


document = catvm.proxy(document);
function code1(x1, difficulty) {
    window.x1 = x1;
